...
update public.app_product_categories
set normalized_label = public.normalize_product_catalog_text(label),
    updated_at = clock_timestamp();

update public.app_product_material_types
set normalized_label = public.normalize_product_catalog_text(label),
    updated_at = clock_timestamp();

update public.app_products
set normalized_name = public.normalize_product_catalog_text(name),
    normalized_description = public.normalize_product_catalog_text(description),
    normalized_specification = public.normalize_product_catalog_text(specification),
    normalized_unit = public.normalize_product_catalog_text(unit),
    updated_at = clock_timestamp();
...
with normalized as (
  select
    ...
    public.normalize_product_catalog_text(concat_ws(' ', product.name, product.specification)) as normalized_description,
    public.normalize_product_catalog_text(product.category_label) as normalized_category,
    public.is_generic_product_description(product.name, product.description, product.specification) as generic_description
