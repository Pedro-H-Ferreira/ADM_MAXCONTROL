...
export async function GET(request: Request) {
  try {
    ...
    const total = resolveFluigUserSyncTotal(states, "my_requests", result.total, { moduleScoped: Boolean(moduleSlug) });

    return NextResponse.json({ success: true, requests: result.requests, total, persistence: result.persistence });
  } catch (error) {
